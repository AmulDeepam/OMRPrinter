...
class InvoiceList1 extends Component {
  ...
  componentDidMount() {   
    ...
    url: "http://52.66.243.218:8080/ERPDetails/saleorder/saleinvoicereport",
   ...
   // self.state.sortTable($('#tableHeadings'),'desc');
    $(".customerId").hide();
    $(".companyName").hide();
          }
          else{
    ...
    $("#tableHeadings").on('click', '#delete', function () {
     ...
     self.state.customerName=currentRow.find("td:eq(3)").text();
  ...
  self.DeleteFunc(currentRow);
    ...
    $("#tableHeadings").on('click', '#view', function () {
      // get the current row
          ...
          self.state.companyName=currentRow.find("td:eq(9)").text(); 
  ...
  DeleteFunc(currentRow)
  {
      ...
      $.ajax({
         ...
         url: "http://52.66.243.218:8080/ERPDetails/SalesReport/DailySalesReportDelete",
          contentType: "application/json",
          dataType: 'json',
          async: false,
