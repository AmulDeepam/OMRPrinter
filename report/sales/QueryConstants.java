...
String DB_CONTEXT_LOOKUP_NAME = "java:jboss/datasources";
	...
	//--------QUERY FOR DAILY SALES REPORT--------------//
		...
		String SALES_REPORT_VIEW = " SELECT customername,contactno,Invoicedate,duedate,productname,size,unit,"
				+"quantity,rate,total,cgsta,sgsta,igsta,finalamount,subtotal1,totalgst,advance,"
				+"discount,height,width,totalcgst,totalsgst,totaligst from SaleInvoiceTable where invoiceNo = ? and companyId = ?  " ;
		...
		String SELECT_YEARLY_SALES_INV = "SELECT  DISTINCT(invoiceNo) FROM SaleInvoiceTable WHERE year(Date) = ? AND companyId= ? and Status = '0' ";
		
		String YERALY_SALES_REPORT = " SELECT SaleInvoiceId,invoiceNo,Date,customerName,contactNo,Payment_status,balance_amount,subtotal1,customerId FROM SaleInvoiceTable WHERE year(DATE) = ? and companyId = ? and invoiceNo = ? limit 1 ";
