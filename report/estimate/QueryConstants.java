...
String DB_CONTEXT_LOOKUP_NAME = "java:jboss/datasources";
	...
	String ESTIMATE_REPORT_VIEW = " SELECT customername,contactno,Invoicedate,duedate,productname,size,unit,"
			...
			+"discount,balance_amount,amount,height,width from EstimateInvoiceTable where invoiceNo = ? and companyId= ? " ;
	...
	String SELECT_YEARLY_ESTIMATE_INV = "SELECT  DISTINCT(invoiceNo) FROM EstimateInvoiceTable WHERE year(Date) = ? AND Status = '0' and companyId= ?  ";

	String YEARLY_ESTIMATE_REPORT = "SELECT invoiceNo,Date,customerName,contactNo,Payment_status,balance_amount,subtotal1,customerId FROM EstimateInvoiceTable WHERE year(Date) = ? and companyId= ? and  invoiceNo = ? limit 1  ";
	...
	String DATE_WISE_ESTIMATE_REPORT = "SELECT invoiceNo,Date,customerName,contactNo,Payment_status,balance_amount,subtotal1,customerId FROM EstimateInvoiceTable WHERE date(Date) BETWEEN ? AND ? and companyId= ? and invoiceNo = ? limit 1  ";
	 ...
	 String Invoice_BalanceAmt="UPDATE EstimateInvoiceTable SET balance_amount = ? where invoiceNo = ? and companyId= ? ";
