...
public interface QueryConstants {
	...
	String PURCHASE_REPORT_VIEW = " SELECT vendorName,contactno,Invoicedate,duedate,productname,size,unit,"
			...
			+"discount,height,width from PurchaseInvoiceTable where invoiceNo = ? and companyId= ? " ;
	...
	String SELECT_YEARLY_PURCHASE_INV = "SELECT  DISTINCT(invoiceNo) FROM PurchaseInvoiceTable WHERE year(Date) = ? AND Status = '0' and companyId=? ";

	String YEARLY_PURCHASE_REPORT = "SELECT invoiceNo,Date,vendorName,contactNo,Payment_status,finalAmountTotal,subtotal1,vendorId FROM PurchaseInvoiceTable WHERE year(Date) = ? and companyId=? and  invoiceNo = ? limit 1 ";
	...
	String Invoice_Amount = "Select sum(finalAmount) as SaleInvoice_Total_Amt from PurchaseInvoiceTable where date(DATE) BETWEEN ? AND ? and vendorId = ?  and companyId= ? ";
