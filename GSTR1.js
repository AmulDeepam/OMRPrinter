...
componentDidMount(){
  ...
  MonthlyFunc(value) {
    ...
    $.ajax({
      ...
      url: "http://52.66.243.218:8080/ERPDetails/gst/businesstobusinessreport",
      contentType: "application/json",
      dataType: 'json',
      async: false,
      success: function (data, textStatus, jqXHR) {
      var tab = '<thead><tr class="headcolor"><th>Invoice No</th><th>Invoice Date</th><th>Vendor Name</th><th>Taxable Value</th><th>CGST</th><th>SGST</th><th>IGST</th><th>Total</th></tr></thead>';
        $.each(data.businesstobusinessreportlist, function (i, item) {
 
          tab += '<tbody id= "myTable" ><tr id="tabletextcol" ><td>' + item.invoiceNo + '</td><td>' + item.invoiceDate + '</td><td>' + item.vendorName + '</td><td>' + item.totalgst + '</td><td>' + item.totalcgst + '</td><td>' + item.totalsgst + '</td><td>' + item.totaligst + '</td><td>' + item.subtotal1 + '</td></tr></tbody>';
        });
