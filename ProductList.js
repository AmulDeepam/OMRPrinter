...
class ProductList1 extends Component {
    constructor(data) {
      ...
      $.ajax({
        ...
        url: "http://52.66.243.218:8080/ERPDetails/master/saleproductreport",
  ...
  if(data.saleProductRetrievelist.length!=0){
    ...
    //var productCategory=currentRow.find("td:eq(9)").html();// get current row 1st TD value
    console.log("cr",productName);
    //console.log("cr",productCategory);
       
    self.state.productName = productName;
   // self.state.productCategory = productCategory;
      ...
      //    productCategory:self.state.productCategory,
        ...
        console.log("console productCategory",self.state.productCategory);
        console.log("console productName",self.state.productName);
        confirmAlert({
          title: 'Delete Confirmation',                        // Title dialog
          message: 'Would you like to delete '+ self.state.productName + ' ? ',               // Message dialog
   ...
   // self.DeleteFunc(currentRow);
      ...
      $("#tableHeadings").on('click', '#edit', function () {
        ...
        self.state.oldProductName = self.state.productName;
        self.state.oldUnit = self.state.unit;
        self.state.oldCgst = self.state.cgst;
        self.state.oldSgst = self.state.sgst;
        self.state.oldIgst = self.state.igst;
        self.state.oldHsnCode = self.state.hsnCode;
        self.state.oldIndividualRate = self.state.individualRate;
        self.state.oldDealerRate = self.state.dealerRate;
        self.state.OldDescription = self.state.description;
        self.state.OldProductCategory = self.state.productCategory;
   ...
   ReactDOM.render(<ProductListEdit productName={self.state.productName}
    ...
    DeleteFunc(currentRow)
    {
        ...
        $.ajax({
          ...
          companyId:self.state.companyId,
           ...
           url: "http://52.66.243.218:8080/ERPDetails/master/deletesaleproduct",
            contentType: "application/json",
            dataType: 'json',
            async: false,
            ...
            success: function (data, textStatus, jqXHR) {
                ...
                currentRow.remove();
                confirmAlert({
                  title: 'Deleted',                        // Title dialog
                  message: ''+ self.state.productName +' deleted successfully',               // Message dialog
                  confirmLabel: 'Ok',                           // Text button confirm
                });
    ...
    <li class="active"><a  style={{color:"black",fontWeight:"bold"}}  className="active" onClick={() => this.SaleProductList()}><span style={{display:"inline-grid"}}>Sale Report</span></a></li>
    <li><a  style={{color:"black",fontWeight:"bold"}}  onClick={() => this.PurchaseProductList()}><span style={{display:"inline-grid"}}>Purchase Report</span></a></li>
...
<ReactHTMLTableToExcel
                    ...
                    table="tableHeadings"
                    filename="Sale_Product_List"
                    sheet="tablexls"
                    buttonText="Download Sale Product"/>
                    </div>
