...
$("#submit").hide();
   ...
   url: "http://52.66.243.218:8080/ERPDetails/PurchaseReport/invoicepaymentreport",
 ...
 var tab = '<thead><tr class="headcolor"><th>Date</th><th>Invoice</th><th>Customer Name</th><th>Due Amount</th><th>Discount</th><th>Pay</th><th>Balance</th></tr></thead>';
  $.each(data.invoicepaymentreportlist, function (i, item) {
    ...
    tab += '<tbody id= "myTable" ><tr id="tabletextcol" ><td>' + item.date + '</td><td>' + item.invoiceNo + '</td><td>' + item.vendorName + '</td><td>' + item.dueAmount + '</td><td>' + item.discount + '</td><td>' + item.pay + '</td><td>' + item.balanceAmt + '</td></tr></tbody>';
  });
...
if((Number(this.state.pay))!=0 && this.state.paymentMode!=''){   
    ...
    $.ajax({
        ...
        url: "http://52.66.243.218:8080/ERPDetails/PurchaseReport/PurchaseReportEdit",
        contentType: "application/json",
        dataType: 'json',
        async: false,
        success: function (data, textStatus, jqXHR) {
